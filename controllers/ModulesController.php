...
class ModulesController extends \yii\easyii2\components\Controller
{
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if(Yii::$app->request->isAjax){
                ...
                if($model->save()){
                    $this->flash('success', Yii::t('easyii2', 'Module created'));
    ...
    public function actionEdit($id)
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            if(Yii::$app->request->isAjax){
                ...
                if($model->save()){
                    $this->flash('success', Yii::t('easyii2', 'Module updated'));
                }
                else{
    ...
    public function actionSettings($id)
    {
        ...
        if (Yii::$app->request->post('Settings')) {
            ...
            if($model->save()){
                $this->flash('success', Yii::t('easyii2', 'Module settings updated'));
            }
            else{
                $this->flash('error', Yii::t('easyii2', Yii::t('easyii2', 'Update error. {0}', $model->formatErrors())));
            }
            return $this->refresh();
        }
        else {
    ...
    public function actionRestoreSettings($id)
    {
        if(($model = Module::findOne($id))){
            $model->settings = '';
            $model->save();
            $this->flash('success', Yii::t('easyii2', 'Module default settings was restored'));
        } else {
            $this->flash('error', Yii::t('easyii2', 'Not found'));
        }
        return $this->back();
    ...
    public function actionCopy($id)
    {
        ...
        if ($formModel->load(Yii::$app->request->post())){
            if(Yii::$app->request->isAjax){
                ...
                $moduleFileContent = str_replace('@easyii2', '@app', $moduleFileContent);
                ...
                foreach(new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator($newModuleFolder)) as $file => $object){
                    if(!$object->isDir()){
                        ...
                        $fileContent = str_replace("Yii::t('easyii2/".$module->name, "Yii::t('easyii2/".$formModel->name, $fileContent);
                ...
                foreach(glob($newModuleFolder . DIRECTORY_SEPARATOR . 'models' . DIRECTORY_SEPARATOR . '*.php') as $modelFile){
                    ...
                    $newTableName = str_replace('easyii2', 'app', $newTableName);
    ...
    public function actionDelete($id)
    {
        ...
        return $this->formatResponse(Yii::t('easyii2', 'Module deleted'));
    }
