 ...
 * @method static void plugin($options = []) Applies FancyBox widget on photos called by box() function
 * @method static string pages() returns pagination html generated by yii\widgets\LinkPager widget.
 * @method static \stdClass pagination() returns yii\data\Pagination object.
 */

class Article extends \yii\easyii2\components\API
{
    private $_cats;
    private $_items;
    private $_adp;
    ...
    public function api_items($options = [])
    {
        if(!$this->_items){
            ...
            $query = $model::find()->with($with)->status(Item::STATUS_ON);
    ...
    public function api_last($limit = 1, $where = null)
    {
        ...
        $model = ActiveRecord::getModelByName('Item', 'article');
        $query = $model::find()->with($with)->status(Item::STATUS_ON)->sortDate()->limit($limit);
        if($where){
    ...
    {
        $model = ActiveRecord::getModelByName('Category', 'article');
        $category = $model::find()->where(['or', 'category_id=:id_slug', 'slug=:id_slug'], [':id_slug' => $id_slug])->status(Item::STATUS_ON)->one();
    ...
    {
        $model = ActiveRecord::getModelByName('Item', 'article');
        $article = $model::find()->where(['or', 'item_id=:id_slug', 'slug=:id_slug'], [':id_slug' => $id_slug])->status(Item::STATUS_ON)->one();
        if($article) {
